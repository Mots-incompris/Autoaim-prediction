...
// 查找轮廓并绘制最小矩形
cv::Mat drawrectangle(cv::Mat dilate,cv::Mat frame)
{
    // canny边缘检测
    cv::Mat edges;
    ...
    for(const auto& contour : contours)
    {
        ...
        // 矩形边框连接
        for(int i=0;i<4;i++)
        cv::line(p,point[i],point[(i+1)%4],cv::Scalar(0,0,255),1);
    }
    return p;
}
...
int main()
 {
    ...
    while (true) 
    {
        ...
        //矩形绘制
        rectangle = drawrectangle(dilateImage,frame);
